...
<form action="../page/quan-tri.php" method="get">
<table><input type="hidden" name="page" value="tkb-sv" />
<tr><td>Nhập mã số sinh viên:</td><td><input style="width:500px" name="seach-sv" type="text" /></td><td><button type="submit" class="btn btn-primary mb-2">Kiểm tra</button></td></tr>
</table>
</form>
			</div>
		</div>
...
<?php
	if(isset($_GET['seach-sv']))
	{
		$masv=$_GET['seach-sv'];
		$strSQL='select * from sinh_vien where masv="'.$masv.'"';
	$kq=mysqli_query($conn, $strSQL);
 	if(mysqli_num_rows($kq)>0)
	{
		echo "<center> <h2>THỜI KHÓA BIỂU THỰC HÀNH CỦA SINH VIÊN $masv</h2></center>";
	$strSQL='select * from dang_ky where masv="'.$masv.'"';
	$kq=mysqli_query($conn, $strSQL);
 	if(mysqli_num_rows($kq)>0)
...
<?php
	...
	else
	{
        ...
        <h3 style="color:#F00">
		<?php
		echo "Sinh viên ".$masv." hiện chưa đăng ký thời khóa biểu thực hành";
		?>
	...
	{
		echo " <center> <h2 style='color:red'>SINH VIÊN $masv KHÔNG TỒN TẠI</h2></center>";
		}
	}
